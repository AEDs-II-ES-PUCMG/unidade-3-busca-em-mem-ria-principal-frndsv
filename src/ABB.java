...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    private void recortar(No<K, V> raizArvore, K chaveDeOnde, K chaveAteOnde, Lista<V> recorte) {
    	...
    	if (raizArvore != null) {
    		comparacaoDeOnde = comparador.compare(chaveDeOnde, raizArvore.getChave()); // O inicio do intervalo e menor ou maior q o no (raiz) atual
    		comparacaoAteOnde = comparador.compare(chaveAteOnde, raizArvore.getChave()); // O fim do intervalo e menor ou maior q o no (raiz) atual
    		if ((comparacaoDeOnde <= 0) && (comparacaoAteOnde >= 0)) { // se tiver dentro do intervalo
                // Faz um caminhamento em ordem normal inserindo a raiz 
    		...
    		} else if (comparacaoDeOnde < 0) { // se o no for maior que o intervalo
    			recortar(raizArvore.getEsquerda(), chaveDeOnde, chaveAteOnde, recorte); // ele vai pra esquerda porque o intervalo ta praquele lado serve pro intervalo 
    		} else {
    			recortar(raizArvore.getDireita(), chaveDeOnde, chaveAteOnde, recorte); // ele vai pra direita porque na esquerda tudo e menor que o intervalo entao nao serve. na direita ainda tem chance de ter algo que atende ao intervalo
    		}
    	}
    }
